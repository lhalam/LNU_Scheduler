...
class ScheduleView(View):
    ...
    def get(self, request):
        if request.GET.get('action')=='add':
            Schedule.add(request.GET.get('day'), request.GET.get('sub_number'), request.GET.get('subject_id'), request.GET.get('teacher_id'), request.GET.get('room_id'), request.GET.get('group_id'))
            items = Schedule.get_by_room_id(request.GET.get('room_id'))
        elif request.GET.get('action')=='remove':
            Schedule.delete_by_id(request.GET.get('id'))
            items = Schedule.get_by_room_id(request.GET.get('room_id'))
        else:
